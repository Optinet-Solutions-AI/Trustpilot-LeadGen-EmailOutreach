...
- **Listing — Apify is now the recommended path** (`YELP_LISTING_SOURCE=apify`; the code's own default is still `browser`, so it must be set explicitly). Cookieless HTTP via `memo23/yelp-scraper` — no browser, no DataDome slider, no sticky IP — so it's the only listing source that runs on Cloud Run and the Linux worker, which is what makes Yelp available to users other than the owner. Measured 2026-08-12: $0.00275/item plus a $0.009 start fee per city run (the 10-item probe cost $0.0365 total, i.e. $0.00365/item at that size — the start fee amortises away on bigger runs), returning rating, review count, phone (10/10), website (8/10), contact email (5/10), claimed status (10/10). One actor call now covers listing AND profile, so `enrich_profiles` does no HTML fetch on this path — only screenshots — and `YELP_MAX_ENRICH` here caps screenshots only (unlimited by default); the legacy paths below still cap data at 25, discarding website/phone past that. Rating filtering is client-side (`searchSortBy` has no ascending option) via `YELP_APIFY_OVERFETCH`/`YELP_APIFY_MAX_ITEMS`. US-only until other markets are probed and added to `YELP_APIFY_MARKETS`. Failure codes: `apify_credit`, `apify_budget_exhausted`, `apify_empty`, `apify_error` (generic failure — preserves leads already gathered from earlier cities), `apify_market_unverified`, plus the pre-existing `filter_too_strict`. Full detail in `workflows/scrape_yelp.md`.
