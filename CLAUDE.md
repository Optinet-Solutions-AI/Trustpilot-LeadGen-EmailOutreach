...
| `YELP_APIFY_OVERFETCH` | Multiplier on the per-city result cap before local rating filtering. Yelp's `searchSortBy` has no ascending option, so low-rated leads are only reachable by over-fetching the Recommended feed and filtering client-side | `4` |
| `YELP_APIFY_MAX_ITEMS` | Ceiling on the over-fetched item count per city, so `YELP_APIFY_OVERFETCH` can't run away with spend | `200` |
