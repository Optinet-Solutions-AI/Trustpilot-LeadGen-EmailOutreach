...
| `YELP_LISTING_SOURCE` | Recommended: `apify` (cookieless, runs on Cloud Run/Linux — see Known Constraints below). The var's own code default is still `browser` (free headed-browser `/search`, owner-local-only). Other values: `fusion` (paid Yelp API, trial expired) and `relay` (residential proxy + human-minted DataDome cookie, fallback escape hatch). | `browser` |
| `APIFY_YELP_ACTOR` | Cookieless Yelp listing+profile actor used by `YELP_LISTING_SOURCE=apify`. Measured 2026-08-12: $0.00275/item plus a $0.009 start fee per city run (the 10-item probe cost $0.0365 total, i.e. $0.00365/item at that size — the start fee amortises away on bigger runs), returning rating, review count, phone (10/10), website (8/10), contact email (5/10), claimed status (10/10) | `memo23/yelp-scraper` |
| `YELP_APIFY_ENRICH_EMAILS` | Passed to the actor as `enrichEmails` — asks it to include a contact email in its output | `true` |
| `YELP_APIFY_OVERFETCH` | Multiplier on the per-city result cap before local rating filtering. Yelp's `searchSortBy` has no ascending option, so low-rated leads are only reachable by over-fetching the Recommended feed and filtering client-side. Measured 2026-08-13 over 233 live businesses: only 16.3% sit at or below the default max_rating of 3.5, so ~6 fetched businesses are needed per usable lead (~$0.017 each) | `6` |
| `YELP_APIFY_MAX_ITEMS` | Ceiling on the over-fetched item count per city, so `YELP_APIFY_OVERFETCH` can't run away with spend. Also sized to fit Apify's HARD 300s server-side cap on the run-sync endpoint at the measured ~2s/item | `100` |
| `YELP_APIFY_MAX_ITEMS_PER_JOB` | Whole-job Apify spend guard — the maximum billable items a single scrape may fetch across all its cities. The per-city ceiling bounds one city; this bounds the fan-out, and it is the ONLY cap on Apify spend in the system. On exhaustion the job stops and emits `apify_budget_exhausted`, keeping the leads already gathered. ~$1.40/job at $0.00275/item plus start fees | `500` |
...
- **Listing — Apify is now the recommended path** (`YELP_LISTING_SOURCE=apify`; the code's own default is still `browser`, so it must be set explicitly). Cookieless HTTP via `memo23/yelp-scraper` — no browser, no DataDome slider, no sticky IP — so it's the only listing source that runs on Cloud Run and the Linux worker, which is what makes Yelp available to users other than the owner. Measured 2026-08-12: $0.00275/item plus a $0.009 start fee per city run (the 10-item probe cost $0.0365 total, i.e. $0.00365/item at that size — the start fee amortises away on bigger runs), returning rating, review count, phone (10/10), website (8/10), contact email (5/10), claimed status (10/10). One actor call now covers listing AND profile, so `enrich_profiles` does no HTML fetch on this path — only screenshots — and `YELP_MAX_ENRICH` here caps screenshots only, never data (default 25 — it was unlimited until a 5-lead request returned 195 and queued 195 screenshots at 75 credits each); the legacy paths below still cap data at 25, discarding website/phone past that. Rating filtering is client-side (`searchSortBy` has no ascending option) via `YELP_APIFY_OVERFETCH`/`YELP_APIFY_MAX_ITEMS`. All 24 seeded markets work (probed 2026-08-14: 22/23 non-US return real businesses; only JP is empty). `YELP_APIFY_MARKETS` now only narrows the list. **But outside the US most listings have no rating at all** (all 133 Austrian roofers measured had none; only 2% of non-US restaurants were ≤3.5 vs 16.3% in the US), and the rating filter drops ratingless rows — so a working market can still yield zero. The `include_unrated` filter keeps them; rating/review floors don't apply to a ratingless row. Pair with `profile_claimed` (an unrated + unclaimed listing is a strong target). Listing emits `all_unrated` rather than `filter_too_strict` when that's the cause. Failure codes: `apify_credit`, `apify_budget_exhausted`, `apify_empty`, `apify_error` (generic failure — preserves leads already gathered from earlier cities), `apify_market_unverified`, plus the pre-existing `filter_too_strict`. Full detail in `workflows/scrape_yelp.md`.
...
- **undetected-chromedriver clears BOTH walls.** Verified 2026-09-24:
  ...
  address, matching our stored rating exactly. Neither needs ScrapingBee.
  Note Yelp draws the profile as a modal over the search page, so crop it the
  way `crop_tripadvisor_header` crops TripAdvisor.
...
- **Every paid path prints `COST:{platform}|{vendor}|{usd}|{units}|{label}`**
  ...
  and totals it in a row beneath the table.
- **Reported as it happens, not at the end**, so a run killed by the watchdog
  still records what it had already spent. A zero-result run that spent money
  is exactly the case this exists for — measured live, a $0.27 TripAdvisor run
  that returned 0 leads.
- **USD is left EMPTY rather than guessed.** OpenAI is exact (the API reports
  token usage per call) and Apify is priced from the actor's listed rate;
